...
#define PMEM_FLAGS_MAP_RO                 0x00010000
...
@Note: For Non Typed memory pool anonymous allocations are not 
allowed for more than single page(4K). As anonymous allocations are lot
slower compare to shmem, to allocate more than 4K size of allocation 
in non typed memory user should use PMEM_FLAGS_SHMEM flag.
==========================================================================*/
...
FUNCTION pmem_alloc_handle_local

@brief
This function creates a PMEM handle out of an input virtual address.

@param [in] vmid_mask  VMID of the GVM that owns the rgn table.
...
int pmem_alloc_handle_local 
( 
  uint32_t vmid_mask, 
  uintptr_t vaddr,
  uint32_t size, 
  pmem_handle_t * OUT handle
  );
...
@param [in] flag attribute for mapping.
            only default cahing attribute PMEM_FLAGS_CACHE_WB_WA supported
			as of now. 
