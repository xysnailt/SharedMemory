...
void CameraHandle::qcarcam_event_cb(qcarcam_hndl_t hndl,
    ...
    if (event_id == QCARCAM_EVENT_FRAME_READY) {
        ...
        qcarcam_ret_t ret = qcarcam_get_frame(
            instance.cam_handle_, &frame_info, QCARCAM_TEST_DEFAULT_GET_FRAME_TIMEOUT, 0);
        if (QCARCAM_RET_OK != ret) {
            std::cout << "qcarcam_get_frame failed errorcode: " << ret << std::endl;
            return;
        }
        if (( unsigned int )frame_info.idx >= instance.cam_buffers_.n_buffers) {
            printf("camera buffer overflow frame index:%d frame buffers:%d\n",
                   frame_info.idx,
                   instance.cam_buffers_.n_buffers);
            return;
        }

        instance.vaddr =
            mmap_device_memory(0,
                               instance.cam_buffers_.buffers[frame_info.idx].planes[0].size,
                               PROT_READ | PROT_WRITE,
                               0,
                               ( uint64_t )instance.physical_address_);
        // std::cout << "phy_addr=" << instance.physical_address_ << " "
        //           << "size=" << instance.cam_buffers_.buffers[frame_info.idx].planes[0].size <<
        //           std::endl;
        if ((MAP_FAILED == instance.vaddr) || (nullptr == instance.vaddr)) {
            std::cout << "mmap_device_memory() failed" << std::endl;
            return;
        }

        // std::cout << "cam_buffers_.buffers[frame_info.idx].planes[0].size: " <<
        // instance.cam_buffers_.buffers[frame_info.idx].planes[0].size << std::endl;
        int buffer_size = instance.cam_buffers_.buffers[frame_info.idx].planes[0].size;

        auto t1 = std::chrono::steady_clock::now();

        memcpy(instance.buff,
               instance.vaddr,
               instance.cam_buffers_.buffers[frame_info.idx].planes[0].size);
        ...
        munmap_device_memory(( void* )instance.physical_address_,
                             instance.cam_buffers_.buffers[frame_info.idx].planes[0].size);

        ret = qcarcam_release_frame(instance.cam_handle_, frame_info.idx);
        if (QCARCAM_RET_OK != ret) {
