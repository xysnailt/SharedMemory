...
int main() {
    ...
    // 打开文件
    std::ifstream file("POSIX_fd.txt");
    if (!file.is_open()) {
    ...
    int fd;
    if (file >> fd) {
        std::cout << "First number in the file: " << fd << std::endl;
    } else {
        std::cerr << "Failed to read first number from file." << std::endl;
        return 1;
    }
    ...
    void *shm_addr = mmap(nullptr, SHARED_MEMORY_SIZE, PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0);
