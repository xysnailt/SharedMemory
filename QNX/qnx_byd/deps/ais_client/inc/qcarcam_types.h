...
typedef enum
{
    ...
    QCARCAM_FMT_YVYU_8 = QCARCAM_COLOR_FMT(QCARCAM_YUV_YVYU, QCARCAM_BITDEPTH_8, QCARCAM_PACK_FOURCC),
    QCARCAM_FMT_VYUY_8 = QCARCAM_COLOR_FMT(QCARCAM_YUV_VYUY, QCARCAM_BITDEPTH_8, QCARCAM_PACK_FOURCC),
...
typedef enum
{
    ...
    QCARCAM_PARAM_GET_HOTPLUG_STATE,    ///< Get hotplug state
    QCARCAM_PARAM_GET_SERDES_CONFIG,    ///< Get serdes config
...
{
    bool hotplug_flag;
}qcarcam_param_hotplug_t;

typedef struct
{
    char module_name[16];   // Module name, RVC, AVM, DMS etc.
    char ser_name[16];      // Serializer chip name
    char des_name[16];      // Deserializer chip name
    char dformat[16];       // Camera output format

    int src_id;
}qcarcam_param_serdes_config_t;
...
typedef union
{
    ...
    qcarcam_param_hotplug_t hotplug_config;
    qcarcam_param_serdes_config_t serdes_config;
