...
static int qcarcam_test_get_time(unsigned long long *pTime)
{
    ...
    *pTime = milliseconds.count();

    std::cout << "Current time in milliseconds: " << milliseconds.count() << " ms\n";
...
static void qcarcam_test_clear_usr_process_list(qcarcam_test_input_t *input_ctxt)
{
    ...
    while (input_ctxt->usr_process_frameCnt)
    {
        ...
        std::this_thread::sleep_for(std::chrono::microseconds(USR_PROCESS_WAIT_USLEEP));
        pthread_mutex_lock(&input_ctxt->mutex);
    }
...
static int check_signal_loss_thread(void *arg)
{
    ...
    while (!g_aborted)
    {
        ...
        std::this_thread::sleep_for(std::chrono::microseconds((signal_check_delay_us)));
    }
    return 0;
}
...
static int check_error_thread(void *arg)
{
    ...
    while (!g_aborted)
    {
        ...
        std::this_thread::sleep_for(std::chrono::microseconds(error_check_delay_us));
    }
    return 0;
}
...
static int framerate_thread(void *arg)
{
    ...
    while (!g_aborted)
    {
        ...
        std::this_thread::sleep_for(std::chrono::microseconds(fps_print_delay_us));
...
static int timer_thread(void *arg)
{
    ...
    while (!g_aborted)
    {
        ...
        std::this_thread::sleep_for(std::chrono::microseconds(TIMER_THREAD_USLEEP));
    }
    return 0;
}
    ...
    if (gCtxt.enablePauseResume && 0 == (input_ctxt->frameCnt % gCtxt.enablePauseResume))
    {
        ...
        if (input_ctxt->state != QCARCAMTEST_STATE_CLOSED)
        {
            ...
            if (ret == QCARCAM_RET_OK)
            {
                ...
                std::this_thread::sleep_for(std::chrono::microseconds(PAUSE_RESUME_USLEEP));
    ...
    else if (gCtxt.enableStartStop && 0 == (input_ctxt->frameCnt % gCtxt.enableStartStop))
    {
        ...
        if (input_ctxt->state != QCARCAMTEST_STATE_CLOSED)
        {
            ...
            if (ret == QCARCAM_RET_OK)
            {
                ...
                std::this_thread::sleep_for(std::chrono::microseconds(START_STOP_USLEEP));
...
static int inpt_ctxt_usr_process_thread(void *arg)
{
    ...
    while (!g_aborted)
    {
        ...
        pthread_mutex_unlock(&input_ctxt->mutex);
        std::this_thread::sleep_for(std::chrono::microseconds(USR_PROCESS_THREAD_USLEEP));
    }
    return 0;
}
