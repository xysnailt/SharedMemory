...
qcarcam_ret_t test_util_dump_window_buffer(test_util_ctxt_t *p_ctxt, test_util_window_t *p_window, unsigned int idx, const char *filename)
{
    ...
    if (0 != fp)
    {
...
#if 0
        std::cout << "===memcpy buffer->size[0]: " << buffer->size[0] << std::endl;
        unsigned char *buff = (unsigned char *)malloc(buffer->size[0]);
        auto t1 = std::chrono::steady_clock::now();
        memcpy(buff, buffer->ptr[0], buffer->size[0]);
        ...
        printf("===memcpy buffer->ptr[0] address: %p\n", &(buffer->ptr[0])); // 打印a的变量地址

        unsigned char *buff2 = (unsigned char *)malloc(buffer->size[0]);
        auto t3 = std::chrono::steady_clock::now();
        memcpy(buff2, buff, buffer->size[0]);
        auto t4 = std::chrono::steady_clock::now();
        long delta2 = std::chrono::duration_cast< std::chrono::milliseconds>(t4 - t3).count();
        std::cout << "===memcpy2 cost time: " << delta2 << "ms" << std::endl;

        unsigned char *buff3 = (unsigned char *)malloc(buffer->size[0]);
        auto t5 = std::chrono::steady_clock::now();
        memcpy(buff3, buff2, buffer->size[0]);
        auto t6 = std::chrono::steady_clock::now();
        long delta3 = std::chrono::duration_cast< std::chrono::milliseconds>(t6 - t5).count();
        std::cout << "===memcpy3 cost time: " << delta3 << "ms" << std::endl;
        printf("===memcpy3 buffer->ptr[0] address: %p\n", &(buff2[0])); // 打印a的变量地址
        ...
        if (n < 20) {
            ...
            output.write(reinterpret_cast<const char*>(buff2), buffer->size[0]);
            output.close();
            n++;
        }
        ...
        free(buff3);
#endif
        std::cout << "p_window->format: " << p_window->format << std::endl;
        if (p_window->format == TESTUTIL_FMT_NV12)
        {
        ...
        {
            std::cout << "numBytesWritten != numByteToWrite: " << p_window->format << std::endl;
