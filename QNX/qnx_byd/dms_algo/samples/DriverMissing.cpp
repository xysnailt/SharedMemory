...
public:
    ...
    void PrintDMSResult(const SDResult& result) {
        ...
        if (MResultValid::RESULT_VALID == driver_miss_result.result_valid) {
            ...
            std::cout << "|DRIVERMISSING:Invalid" << std::endl;
        }
    }
};
...
int Init(DMSConfig& para) {
    ...
    ret = SenseDriverLoadModule(*para.handle, bitmask & SENSEDRIVER_MODULE_EMO_MOUTH, false);
    if (MDMS_SUCCESS != ret) {
        std::cerr << "SenseDriverLoadModule error" << std::endl;
        return -1;
    }
    ret = SenseDriverLoadModule(*para.handle, bitmask & SENSEDRIVER_MODULE_GAZE_AOI, false);
    if (MDMS_SUCCESS != ret) {
        std::cerr << "SenseDriverLoadModule error" << std::endl;
        return -1;
    }
    ret = SenseDriverLoadModule(*para.handle, bitmask & SENSEDRIVER_MODULE_SMOKE, false);
    if (MDMS_SUCCESS != ret) {
        std::cerr << "SenseDriverLoadModule error" << std::endl;
        return -1;
    }
