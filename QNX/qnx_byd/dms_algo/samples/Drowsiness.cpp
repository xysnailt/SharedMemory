    ...
    std::string Drowsiness2Str(const MDrowsinessLevel& status) {
        switch (status) {
            ...
            case MDrowsinessLevel::M_DROWSINESS_LEVEL_HEAVY:
                return "Heavy";
            case MDrowsinessLevel::M_DROWSINESS_LEVEL_MICRO_SLEEP:
                return "Micro Sleep";
            case MDrowsinessLevel::M_DROWSINESS_LEVEL_SLEEP:
                return "Sleep";
            case MDrowsinessLevel::M_DROWSINESS_LEVEL_UNRESPONSIVE:
                return "Unreponsive";
            default:
                return "Invalid";
        }
    ...
    void PrintDMSResult(const SDResult& result) {
        ...
        if (MResultValid::RESULT_INVALID == face_result.result_valid) {
            ...
            std::cout << "|FACE_RECT:"
                      ...
                      << "]";
            std::cout << "|FACE_HEADPOSE:" << face_result.face_align_result.head_pose.yaw << ","
                      << face_result.face_align_result.head_pose.pitch << ","
                      << face_result.face_align_result.head_pose.roll;
            ...
            if (MResultValid::RESULT_VALID == drowsiness_result.result_valid) {
                ...
                std::cout << ", |LEFT_EYE_REALTIME: "
                          << EyeStatus2Str(drowsiness_result.eye_left_rt);
                std::cout << ", |RIGHT_EYE_REALTIME: "
                          << EyeStatus2Str(drowsiness_result.eye_right_rt);
                std::cout << ", |LEFT_EYE_SMOOTH:: "
                          << EyeStatus2Str(drowsiness_result.eye_left_sm);
                std::cout << ", |RIGHT_EYE_SMOOTH: "
                          << EyeStatus2Str(drowsiness_result.eye_right_sm);
                std::cout << ", |MOUTH_REALTIME: " << MouthStatus2Str(drowsiness_result.mouth_rt);
                std::cout << ", |MOUTH_SMOOTH: " << MouthStatus2Str(drowsiness_result.mouth_sm)
                          << std::endl;
            } else {
...
int DoDMS(DMSConfig& para) {
    ...
    // use input fps, not video fps, because NIO video fps is strange
