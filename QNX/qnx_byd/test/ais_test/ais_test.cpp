...
int main(int argc, char *argv[])
{
    ...
    {
        printf("Failed qcarcam_query_inputs with ret %d %d %d", query_ret2, queryFilled, queryNumInputs);
        exit(-1);
    }
    ...
    // buffers
    cam_buffers_.buffers = (qcarcam_buffer_t *)calloc(cam_buffers_.n_buffers, sizeof(*cam_buffers_.buffers));
    ...
    if (!virtual_address_)
    {
        printf("camera_id:{}, new for virtual_address_ failed, buf num: {}", 0,
    ...
    {
        printf("camera_id:{}, new for physical_address_ failed, buf num: {}", 0,
               cam_buffers_.n_buffers);
        return -1;
    }
    for (int i = 0; i < cam_buffers_.n_buffers; ++i)
    {
        ...
        virtual_address_[i] = pmem_malloc_ext_v2(
            cam_buffers_.buffers[i].planes[0].size, PMEM_CAMERA_ID,
            PMEM_FLAGS_SHMEM | PMEM_FLAGS_PHYS_NON_CONTIG | PMEM_FLAGS_CACHE_NONE,
            PMEM_ALIGNMENT_4K, 0, &mem_handle, NULL);
        cam_buffers_.buffers[i].planes[0].p_buf = mem_handle;
        physical_address_[i] = pmem_get_phys_addr(virtual_address_[i]);
    }
